...
```
<subject>.knowledge/
  bundle.yaml        # manifest: identity + boundary policy (required)
...
**`skills/` (and `design/`) — the same "does this belong in the bundle or the console" question, answered by a different test.** `sources/ledger/journal` split by *voice*; `skills/` splits by a different axis: **does this capability make sense independent of which specific subject's bundle it's operating on, or does it only exist because of something particular to *this* subject?**

- **Generic operating mechanism** (a daily-briefing routine, a session-wrap-up ritual, a weekly-planning cadence) works the same way regardless of *which* subject's bundle is mounted — it's a property of the *console*, not the bundle. Stays in the console's own tooling, never in `skills/`.
- **Subject-specific capability** (a brand-voice checker that only makes sense because of one brand's actual voice rules, a person's own writing-voice-matching skill) exists *because of* facts particular to this one subject. It belongs in that subject's own `skills/`, because it's part of what makes the bundle a complete, portable artifact — hand the bundle to a new owner (a client bundle changing hands, per §2.3's "operable assets" on archive) and the skill needs to travel with it, the same way a brand's actual voice guidelines do.

The test isn't "is this a skill" — plenty of subject-specific skills exist. It's "would this capability still make sense if you swapped which subject's bundle was mounted." If yes, it's console tooling. If no — it only works because of *this* subject — it's bundle-resident.
...
## 5. The manifest — `bundle.yaml` *(draft)*
...
- **`subject:`** — **kebab-case, lowercase, always**, multi-word or not (`turbotax`, `creative-systems-studio`). The bundle's repo name mirrors it directly: `<subject>.knowledge`. One slug, one casing rule, no separate naming decision to make per bundle — a multi-word subject is not a special case.
...
- **Extends OKF with:** the subject-type / perspective model (§2), the `BOUNDARY` / `rises:` policy, `classification` metadata, and the bundle profile (manifest + the optional design/skills/decisions layers).
...
- **Narrowed 2026-07-13** — which side of the console/bundle line a skill sits on is resolved (§3: does it make sense independent of which subject's bundle is mounted). Still open: how a subject-specific skill that proves broadly useful gets *promoted* into a shared/agency skill library without just copying the client-specific file — the rise mechanism for capability, parallel to `rises:` for knowledge but not the same thing.
...
- **Bundle-approver identity.** §4 requires an append-only affirmation event to move a `type: decision` document's `status:` past `proposed`, but the spec never says *who* is authorized to write one, and the manifest (§5) has no approver/owner field. The identity must never be hardcoded into the bundle — a name written into `bundle.yaml` (`approver: jane-doe`) goes stale the moment that person leaves the role. The correct shape is a role reference the bundle stores, resolved dynamically against whatever the mounting console's actual source of truth for role-holders is (a roster, an org chart, an RBAC system) — not a name the format itself owns or freezes. What affirmation needs from that system is one thing: a live answer to "who currently holds the approver role for this bundle," asked at the moment of affirmation, not stored in advance. If no one currently holds that role, decisions in the bundle stay `proposed` forever — the correct and honest outcome, not a bug to route around. How role-resolution actually works is the mounting console's architecture to own, not this format's to prescribe.
...
- **"soul.md" portability, considered 2026-07-14, not designed.** Raised as a question of whether adopting other agent-persona standards (SOUL.md-style files, "heartbeat" mechanisms) would help a `.knowledge` bundle port into other agent runtimes (OpenClaw/Hermes-family tools were named specifically). Tentative split, not yet a design: **memory** is already this format's whole job — no separate concept needed. **A SOUL.md-shaped file is plausibly a legitimate *export adapter*** — a rendered view compiled from an already-conformant `person` bundle (stable identity from `wiki/`+`decisions`, volatile texture from `stm/`), the same relationship OKF already has to this spec (an export target, not a thing this spec's core needs to grow new sections for). Not built. **"Heartbeat" (a runtime wake/tick cadence) fails the console/bundle test outright** — it doesn't depend on which subject's bundle is mounted, so it's runtime/console architecture, not knowledge content, and doesn't belong in this spec at all. Nobody who actually uses OpenClaw/Hermes has validated any of this against real requirements yet — treat the whole thread as a hunch worth revisiting if a real port is attempted, not a direction this spec has committed to.
