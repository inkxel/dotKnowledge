...
- **Where does the affirmation event itself live?** §4's append-only event has no stated address — §3's bundle layout doesn't reserve a file for it, unlike OKF's own `log.md` (§7 of the OKF spec, a reserved bundle-history file). Once [knowledge-catalog#148](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/148)'s typed-edge/resolution-event convention lands upstream (converged 2026-07-12: resolution events for `supersedes`/`contested_by` live in OKF's `log.md`, keyed to stable concept ids), decide whether `.knowledge`'s own `status:` affirmation event should follow the same convention rather than inventing a second, unlocated mechanism for the same shape of problem.
