...
# skills/ console-vs-capsule test resolved; checked PR #195 for impact

## Context
Tucker caught a real contradiction in earlier framing: skills/tooling was said to stay in the console (never the capsule), but `client-knowledge-template` already has its own `skills/` and §3's bundle layout already lists `skills/` as an optional capsule layer. Also asked whether GitHub PR #195 (GoogleCloudPlatform/knowledge-catalog) — new movement on the #148 typed-edges thread this whole session has been engaged in — affects any current work.

## What changed
- §3: added the actual test for which side of the console/capsule line a skill sits on — does the capability make sense independent of which subject's bundle is mounted (console-side: daily-briefing routines, session-wrap-up rituals) vs. does it only exist because of facts particular to *this* subject (capsule-resident: a brand's own voice-checker, a person's own voice-matching skill) — because it has to survive a handoff, same as a brand's actual voice guidelines would.
...
## Decisions made
- The console/capsule test for skills (independent-of-subject vs. subject-specific) — a real design decision, not just documentation.
